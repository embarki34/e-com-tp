...
import { LayoutDashboard, Package, ShoppingCart, ChevronLeft, ChevronRight, Sun, Moon } from 'lucide-react';
...
const Dashboard = () => {
  ...
  return (
    <div className={cn("flex h-screen", theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100')}>
      <aside 
        className={cn(
      ...
      >
        <div className={cn(
          ...
          {!isSidebarCollapsed && (
            <h2 className={cn("text-xl font-bold flex items-center", theme === 'dark' ? 'text-blue-400' : 'text-blue-600')}>
        ...
        <div className={cn(
          ...
          >
            {theme === 'dark' ? (
              <Sun className={cn(isSidebarCollapsed ? "h-8 w-8" : "h-5 w-5")} />
            ) : (
              <Moon className={cn(isSidebarCollapsed ? "h-8 w-8" : "h-5 w-5")} />
            )}
          </Button>
